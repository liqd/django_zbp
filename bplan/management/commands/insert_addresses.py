...
def parse_fis_broker_address_file(file):
    ...
    StreetNumberCollector = create_tag_text_collector("designator", multiple=True)
    ...
    for event, elem in tqdm(
        etree.iterparse(file, events=("start", "end", "start-ns", "end-ns"))
    ):
        ...
        for comp_key, comp_values in parsed["components"].items():
            ...
            for part in parsed.keys():
                ...
                if not values or not values[0]:
                    print(
                        "No value for {} at component {}".format(
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        addresses_fixture = os.path.join(
            settings.BASE_DIR, "bplan", "fixtures", "addresses.gml"
        )
        parser.add_argument(
            "gml_file",
